...
export default function Home() {
  ...
  useEffect(() => {
    async function init() {
      ...
      setStatus(found.length === 8
        ...
        : `⚠️ ${found.length}/8 tables accessible`);
      setLoading(false);
    }
    init();
  }, [router]);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', system-ui, sans-serif", background: "#F4F6F8", padding: 24 }}>
      ...
      <p style={{ fontSize: 13, color: "#5D6D7E", marginBottom: 24 }}>
        Logged in as <strong>{user?.email}</strong>
      </p>
      <div style={{ background: "#fff", borderRadius: 16, padding: 24, maxWidth: 500, width: "100%", border: "1px solid #E8EAED", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
        ...
        <button onClick={handleLogout} style={{ marginTop: 20, width: "100%", padding: "10px", borderRadius: 10, border: "1px solid #E8EAED", background: "transparent", fontSize: 13, color: "#5D6D7E", cursor: "pointer", fontFamily: "inherit" }}>
          Log out
        </button>
      </div>
