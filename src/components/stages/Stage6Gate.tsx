// Stage6Gate.tsx — Ported from Level 1 Stage6DecisionGate.jsx
'use client';
// @ts-nocheck
import { useState, useEffect, useRef } from "react";
import { THEMES, getTheme, USER } from "@/lib/theme";
const STORAGE_KEY = "dk-stage6-gate";
...
export default function Stage6Gate(){
  ...
  return(<div style={{maxWidth:940,margin:"0 auto",padding:mobile?"20px 16px":"32px 24px",fontFamily:"'DM Sans',sans-serif"}}>
    ...
    <div style={{marginBottom:20,borderBottom:"3px solid #1ABC9C",paddingBottom:16}}>
      ...
      <p style={{fontSize:14,color:t.textMuted,margin:0}}>Does the data support scaling beyond the beachhead?</p>
    </div>
    <div style={{marginBottom:16,padding:"10px 14px",background:t.card,border:"1px solid "+t.cardBorder,borderRadius:10,display:"flex",alignItems:"center",gap:12}}>
      ...
      <div style={{flex:1,height:8,background:t.input,borderRadius:4,overflow:"hidden"}}><div style={{width:(passCount/READINESS.length*100)+"%",height:"100%",background:"#1ABC9C",borderRadius:4}}/></div>
      <span style={{fontSize:13,fontWeight:700,color:"#1ABC9C",fontFamily:"'DM Mono',monospace"}}>{passCount}/{READINESS.length}</span>
    ...
    <div style={{display:"flex",gap:4,marginBottom:16}}>
      {TABS.map((l,i)=><button key={i} onClick={()=>setTab(i)} style={{padding:"8px 16px",borderRadius:8,fontSize:12,fontWeight:tab===i?700:500,cursor:"pointer",border:"1px solid "+(tab===i?"#1ABC9C":t.cardBorder),background:tab===i?"#1ABC9C10":"transparent",color:tab===i?"#1ABC9C":t.textMuted}}>{l}</button>)}
    </div>
    ...
    {tab===2&&<div style={{padding:"16px 18px",background:t.card,border:"1px solid "+t.cardBorder,borderRadius:14}}>
      ...
      {state.decision&&<div style={{marginTop:12,padding:"12px 16px",background:state.decision==="go"?"#1B9C8510":state.decision==="pivot"?"#E67E2210":"#E74C3C10",borderRadius:10}}>
        <p style={{fontSize:13,fontWeight:700,color:state.decision==="go"?"#1B9C85":state.decision==="pivot"?"#E67E22":"#E74C3C",margin:"0 0 4px"}}>Stage 6 Verdict: {state.decision.toUpperCase()}</p>
    ...
    <div style={{display:"flex",gap:8,marginTop:16}}>
      <button onClick={()=>{navigator.clipboard.writeText(JSON.stringify({stage:6,gate:state,exported_at:new Date().toISOString()},null,2));setToast("Copied")}} style={{padding:"12px 20px",background:"#1ABC9C",color:"#fff",border:"none",borderRadius:10,fontSize:14,fontWeight:600,cursor:"pointer",flex:1,minHeight:48}}>Export Gate</button>
    </div>
